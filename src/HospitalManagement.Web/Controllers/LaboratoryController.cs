...
public class LaboratoryController : Controller
{
    ...
    public async Task<IActionResult> EditTest(int id) { var test = await _laboratory.GetTestByIdAsync(id); if (test is null) return NotFound(); ViewData["FormAction"] = nameof(EditTest); ViewData["Title"] = "Edit laboratory test"; return View("CreateTest", new SaveLabTestDto { TestCode=test.TestCode, TestName=test.TestName, Category=test.Category, Price=test.Price, NormalRange=test.NormalRange, IsActive=test.IsActive }); }
    [HttpPost, ValidateAntiForgeryToken] public async Task<IActionResult> EditTest(int id, SaveLabTestDto model) { if (!ModelState.IsValid) { ViewData["FormAction"] = nameof(EditTest); return View("CreateTest", model); } try { await _laboratory.SaveTestAsync(model, id); TempData["SuccessMessage"] = "Lab test updated."; return RedirectToAction(nameof(Index)); } catch (Exception e) when (e is InvalidOperationException or KeyNotFoundException) { ModelState.AddModelError(string.Empty, e.Message); ViewData["FormAction"] = nameof(EditTest); return View("CreateTest", model); } }
